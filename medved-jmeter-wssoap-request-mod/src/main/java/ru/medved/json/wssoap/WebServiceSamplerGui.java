 ...
 * 
 */
public class WebServiceSamplerGui extends AbstractSamplerGui implements
		java.awt.event.ActionListener {
	private static final Logger log = LoggingManager.getLoggerForClass();

	private static final long serialVersionUID = 240L;

	private final JLabeledTextField domain = new JLabeledTextField(
			JMeterUtils.getResString("web_server_domain")); // $NON-NLS-1$

	private final JLabeledTextField protocol = new JLabeledTextField(
			JMeterUtils.getResString("protocol"), 4); // $NON-NLS-1$

	private final JLabeledTextField port = new JLabeledTextField(
			JMeterUtils.getResString("web_server_port"), 4); // $NON-NLS-1$

	private final JLabeledTextField path = new JLabeledTextField(
			JMeterUtils.getResString("path")); // $NON-NLS-1$

	private final JLabeledTextField soapAction = new JLabeledTextField(
			JMeterUtils.getResString("webservice_soap_action")); // $NON-NLS-1$

	private JTextArea soapXml;

	private final JLabeledTextField wsdlField = new JLabeledTextField(
			JMeterUtils.getResString("wsdl_url")); // $NON-NLS-1$

	private final JButton wsdlButton = new JButton(
			JMeterUtils.getResString("load_wsdl")); // $NON-NLS-1$

	private final JButton selectButton = new JButton(
			JMeterUtils.getResString("configure_wsdl")); // $NON-NLS-1$

	private JLabeledChoice wsdlMethods = null;

	private transient WSDLHelper HELPER = null;

	private final FilePanel soapXmlFile = new FilePanel(
			JMeterUtils.getResString("get_xml_from_file"), ".xml"); // $NON-NLS-1$

	private final JLabeledTextField randomXmlFile = new JLabeledTextField(
			JMeterUtils.getResString("get_xml_from_random")); // $NON-NLS-1$

	private final JLabeledTextField connectTimeout = new JLabeledTextField(
			JMeterUtils.getResString("webservice_timeout"), 4); // $NON-NLS-1$

	private final FilePanel jsonConfigFile = new FilePanel("JSON Config file",
			".json"); // $NON-NLS-1$

	private final JLabeledTextField configSection = new JLabeledTextField(
			"Config section"); // $NON-NLS-1$

	/**
	 * checkbox for memory cache.
	 */
	private JCheckBox memCache = new JCheckBox(
			JMeterUtils.getResString("memory_cache"), true); // $NON-NLS-1$

	/**
	 * checkbox for reading the response
	 */
	private JCheckBox readResponse = new JCheckBox(
			JMeterUtils.getResString("read_soap_response")); // $NON-NLS-1$

	/**
	 * checkbox for use proxy
	 */
	private JCheckBox useProxy = new JCheckBox(
			JMeterUtils.getResString("webservice_use_proxy")); // $NON-NLS-1$
	 ...
	 * Text note for proxy
	 */
	private String proxyToolTip = JMeterUtils
			.getResString("webservice_proxy_note") // $NON-NLS-1$
			+ " " // $NON-NLS-1$
	...
	public String getLabelResource() {
		return "medved_webservice_sampler_title"; // $NON-NLS-1$
	}
	...
	public void modifyTestElement(TestElement s) {
		...
		boolean configEnabled = configSection.getText().length() > 0
				&& jsonConfigFile.getFilename().length() > 0;

		if (configEnabled) {
			String section = configSection.getText();
			String fileName = jsonConfigFile.getFilename();
			ConfigReader reader = new ConfigReader();
			WSSOAPConfig cfg = reader.readConfig(fileName, section);
			log.info("|==[CFG]=====================================");
			log.info("| Server/Domain : " + cfg.getServer());
			log.info("| Port          : " + cfg.getPort());
			log.info("| Protocol      : " + cfg.getProtocol());
			log.info("| Path prefix   : " + cfg.getPrefix());
			log.info("| Path          : " + cfg.getPath());
			String fullPath = cfg.getPrefix() + cfg.getPath();
			log.info("| Full path     : " + cfg.getPath());
			log.info("| WSDL URL      : " + cfg.getWsdlUrl());
			log.info("| SOAP Action   : " + cfg.getSoapAction());
			log.info("|============================================");

			sampler.setDomain(cfg.getServer());
			sampler.setProperty(HTTPSamplerBase.PORT, cfg.getPort());
			sampler.setProtocol(cfg.getProtocol());
			sampler.setPath(fullPath);
			sampler.setWsdlURL(cfg.getWsdlUrl());
			sampler.setSoapAction(cfg.getSoapAction());
			sampler.setConfigSection(section);
			sampler.setJsonConfig(fileName);
		} else {
	...
	public void clearGui() {
		...
		jsonConfigFile.setFilename("");
		configSection.setText("");
	}
	...
	private final JPanel createTopPanel() {
		...
		wsdlHelper.setBorder(BorderFactory.createTitledBorder(
		...
		JLabel selectLabel = new JLabel(
				JMeterUtils.getResString("webservice_methods")); // $NON-NLS-1$
	...
	private final JPanel createMessagePanel() {
		...
		soapXmlPane.setBorder(BorderFactory.createTitledBorder(JMeterUtils
				.getResString("soap_data_title"))); // $NON-NLS-1$
		...
		randomXmlPane.setBorder(BorderFactory.createTitledBorder(JMeterUtils
				.getResString("webservice_get_xml_from_random_title"))); // $NON-NLS-1$
	...
	public void configure(TestElement el) {
		...
		jsonConfigFile.setFilename(sampler.getJsonConfig());
		configSection.setText(sampler.getConfigSection());
	...
	public void configureFromWSDL() {
		if (HELPER != null) {
			...
			this.soapAction.setText(HELPER.getSoapAction(this.wsdlMethods
					.getText()));
		}
	}
	...
	public String[] browseWSDL(String url, boolean silent) {
		try {
			...
			WebServiceSampler sampler = (WebServiceSampler) this
					.createTestElement();
